...
use crate::{
    IdentitiesKeys, IdentitiesReader, Identity, IdentityError, Purpose, PurposeKeysVerification,
    SecureChannelPurposeKey, SecureChannelPurposeKeyBuilder, SecureChannelPurposeKeyKey,
    SecureChannelPurposeKeyOptions, Vault,
};

/// This struct supports all the services related to identities
#[derive(Clone)]
pub struct SecureChannelPurposeKeysCreation {
    vault: Vault,
...
impl SecureChannelPurposeKeysCreation {
    ...
    pub fn purpose_key_builder(&self, identifier: &Identifier) -> SecureChannelPurposeKeyBuilder {
...
impl SecureChannelPurposeKeysCreation {
    ...
    pub async fn create_purpose_key_with_options(
        ...
        let (secret_key, public_key) = match options.key {
            SecureChannelPurposeKeyKey::Secret(key) => {
                let public_key = self
                    ...
                    .get_x25519_public_key(&key)
                    .await?;
                (key, public_key)
            }
            SecureChannelPurposeKeyKey::Public(_) => {
                return Err(IdentityError::ExpectedSecretKeyInsteadOfPublic.into());
            }
        };

        attestation_options.key = SecureChannelPurposeKeyKey::Public(public_key.clone());
        ...
        self.repository
            .set_purpose_key(&identifier, Purpose::SecureChannel, &attestation)
            .await?;
    ...
    pub async fn attest_purpose_key(
        ...
        let public_key = match options.key {
            SecureChannelPurposeKeyKey::Secret { .. } => {
                return Err(IdentityError::ExpectedPublicKeyInsteadOfSecret.into())
            }
            SecureChannelPurposeKeyKey::Public(public_key) => public_key,
        };

        let public_key = PurposePublicKey::SecureChannelStaticKey(public_key);
    ...
    pub async fn import_purpose_key(
        ...
        let (key_id, public_key) = match purpose_key_data.public_key.clone() {
            ...
            PurposePublicKey::CredentialSigningKey(_public_key) => {
                panic!() // FIXME
            }
        };
