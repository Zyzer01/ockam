...
impl AwsKmsClient {
    ...
    fn cast_handle_to_kid(key: &SigningSecretKeyHandle) -> Result<String> {
        let key = match key {
            SigningSecretKeyHandle::EdDSACurve25519(_) => {
                panic!()
            } // FIXME
            SigningSecretKeyHandle::ECDSASHA256CurveP256(value) => value.value(),
        };

        Ok(String::from_utf8(key.clone()).unwrap()) // FIXME
    }
